...
class ModelBuilder(nn.Module):
  ...
  def forward(self, input_dict):
    ...
    lr, hr, rec_targets, rec_lengths = input_dict['lr'], \
                                       input_dict['hr'], \
    ...
    if self.STN_ON:
      ...
      stn_input = F.interpolate(lr, self.tps_inputsize, mode='bilinear', align_corners=True)
      stn_img_feat, ctrl_points = self.stn_head(stn_input)
      lr, _ = self.tps(lr, ctrl_points)
      hr,_ = self.tps(hr, ctrl_points)
      if not self.training:
        # save for visualization
        return_dict['output']['ctrl_points'] = ctrl_points
        return_dict['output']['rectified_images'] = lr

    x = self.sub_mean(lr)
    encoder_feats, sharing_feats = self.encoder(x)
    encoder_feats = encoder_feats.contiguous()

    if self.training:
      fake_HR = self.rcan(sharing_feats)
      ...
      return_dict['losses']['loss_sr'] = loss_sr
